...
import { FaClock, FaCheckCircle, FaCalendarDay } from 'react-icons/fa';
...
function EmployeeDashboard() {
  ...
  const fetchStatus = async () => {
    try {
      ...
      // ✅ no /api here, baseURL handles it
      const response = await api.get('/attendance/today', config);
  ...
  const handleCheckIn = async () => {
    try {
      const token = user.token;
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await api.post('/attendance/checkin', {}, config); // ✅ api instead of axios
  ...
  const handleCheckOut = async () => {
    try {
      ...
      const token = user.token;
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await api.post('/attendance/checkout', {}, config); // ✅ api instead of axios
      ...
      toast.error(
        error.response?.data?.message ||
          'Check Out Failed: Review Server Logs.'
      );
    }
  };
      ...
      <div style={cardStyle}>
        ...
        <div style={{ margin: '30px 0' }}>
          {!todayStatus ? (
            ...
            >
              <FaCheckCircle style={{ marginRight: '8px' }} /> Work Day
              Completed
            </div>
          ) : (
            <div
